...
function PatientDoctorDetailsContent() {
  ...
  const loadSlots = useCallback(async () => {
    ...
    try {
      setSlots(
        await getPatientDoctorSlots(session.access_token, doctorId, from, to),
      );
      ...
      setError(
        slotError instanceof Error
          ? slotError.message
          : "Online consultation times could not be loaded.",
      );
    } finally {
      ...
      try {
        ...
        setError(
          loadError instanceof Error
            ? loadError.message
            : "The doctor profile could not be loaded.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const slotsByDate = visibleSlots.reduce(
    (acc, slot) => {
      const dateStr = new Date(slot.startsAt).toLocaleDateString(undefined, {
        weekday: "short",
        day: "numeric",
        month: "short",
      });
  ...
  async function requestAppointment(event: FormEvent) {
    ...
    setError(null);
    try {
      await createPatientAppointment(session.access_token, {
              ...
              {slotsLoading ? (
                ...
                <div className="space-y-6 max-h-125 overflow-y-auto pr-2">
                  {Object.entries(slotsByDate).map(([date, daySlots]) => (
                    <div key={date}>
                      <h4 className="text-sm font-semibold text-slate-900 mb-3 sticky top-0 bg-white py-1">
                        {date}
                      </h4>
          ...
          <form onSubmit={requestAppointment} className="space-y-6 max-w-3xl">
            <div className="space-y-1">
              ...
              <Input
                ...
                onChange={(e) =>
                  setBooking({ ...booking, reasonForVisit: e.target.value })
                }
              />
            </div>
            ...
            <div className="space-y-1">
              <Label htmlFor="symptoms">
                Symptoms <span className="text-rose-600">*</span>
              </Label>
              <Textarea
                id="symptoms"
                maxLength={2000}
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setBooking({ ...booking, symptomDuration: e.target.value })
                  }
                />
              </div>
              <div className="space-y-1">
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    setBooking({ ...booking, additionalNotes: e.target.value })
                  }
                />
              </div>
            </div>
