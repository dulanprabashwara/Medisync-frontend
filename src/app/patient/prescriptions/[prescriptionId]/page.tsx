...
function Content() {
  ...
  return <main className="mx-auto max-w-5xl px-6 py-10 lg:px-8 lg:py-14">
    ...
    {value ? <>
      ...
      {!cancelled ? <section className={`mt-6 rounded-3xl border p-6 ${value.doctorFeeStatus === "AWAITING_CONFIRMATION" ? "border-amber-200 bg-amber-50 text-amber-950" : "border-emerald-200 bg-emerald-50 text-emerald-950"}`}><p className="text-xs font-bold uppercase tracking-[0.16em]">Doctor consultation fee</p><h2 className="mt-2 text-2xl font-semibold">{value.doctorFeeCurrency} {Number(value.doctorFeeAmount).toFixed(2)}</h2><p className="mt-2 text-sm">{value.doctorFeeStatus === "NOT_REQUIRED" ? "No payment confirmation is required for this prescription." : value.doctorFeeStatus === "CONFIRMED" ? "Your doctor confirmed receipt. QR generation is unlocked." : "Awaiting doctor confirmation. Your prescription QR stays locked until the doctor records payment receipt."}</p>{value.doctorFeeAmount > 0 ? <div className="mt-4 border-t border-amber-200/50 pt-4 text-sm"><p className="font-semibold">Doctor payment details:</p><p className="mt-1">Account Holder: {value.doctorBankAccountHolder || "N/A"}</p><p>Bank: {value.doctorBankName || "N/A"}</p><p>Branch: {value.doctorBankBranch || "N/A"}</p><p>Account Number: {value.doctorBankAccountNumber || "N/A"}</p></div> : null}{value.doctorFeeStatus === "AWAITING_CONFIRMATION" ? <Link href={`/patient/consultations/${value.consultationId}`} className="mt-4 inline-flex rounded-xl border border-amber-700 px-4 py-2 text-sm font-semibold">Open consultation chat</Link> : null}</section> : null}
