  ...
  return <main className="mx-auto max-w-5xl px-6 py-10 lg:px-8 lg:py-14">
    ...
    {value ? <>
      <section className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <div className="flex justify-between gap-4"><div><h2 className="text-xl font-semibold">{value.doctorName}</h2><p className="mt-1 text-sm text-slate-600">{value.medicalRegistrationNumber} · {value.specializationName}</p><p className="text-sm text-slate-600">{value.hospitalName} · {value.departmentName}</p></div><PrescriptionStatusBadge status={value.status} expired={value.expired} /></div>
