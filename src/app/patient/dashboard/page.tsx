...
import { Calendar, Search, Activity, FileText, AlertCircle, CheckCircle2 } from "lucide-react";
...
function PatientDashboardContent() {
  ...
  const { upcoming, pendingRequests, history } = useMemo(() => {
    ...
    return {
      upcoming: appointments.filter(a => (a.status === "CONFIRMED" || a.consultationStatus === "IN_PROGRESS" || a.consultationStatus === "SCHEDULED") && (new Date(a.scheduledStart).getTime() > now || a.consultationStatus === "IN_PROGRESS")).sort(asc),
      pendingRequests: appointments.filter(a => a.status === "REQUESTED").sort(asc),
      history: appointments.filter(a => a.consultationStatus === "COMPLETED" || (a.status !== "REQUESTED" && a.status !== "CONFIRMED" && a.consultationStatus !== "IN_PROGRESS" && a.consultationStatus !== "SCHEDULED")).sort(desc),
    };
  }, [appointments, now]);
  ...
  const { pendingPayments, activePrescriptions } = useMemo(() => {
    return {
      ...
      activePrescriptions: prescriptions.filter(p => p.status === "ISSUED" && p.dispensingStatus === "NOT_DISPENSED" && !p.expired),
    };
  }, [prescriptions]);
        ...
        <div className="space-y-8">
          <div className="grid gap-6 lg:grid-cols-3">
                  ...
                  <div className="flex flex-col sm:flex-row gap-6">
                    <div className="flex-1">
                      ...
                      <div className="mt-4 space-y-1">
                        ...
                        <p className="text-sm text-slate-600">{nextConsultation.hospitalName}</p>
                      </div>
                    </div>
                    <div className="flex flex-col items-start sm:items-end gap-3">
                      <StatusBadge tone={nextConsultation.consultationStatus === "IN_PROGRESS" ? "info" : "success"}>
            ...
            <div className="space-y-6">
              ...
              <StatCard label="Pending Requests" value={pendingRequests.length} icon={Activity} />
              <StatCard label="Active Prescriptions" value={activePrescriptions.length} icon={FileText} />
            </div>
          </div>
              ...
              <div className="divide-y divide-slate-100">
                {history.slice(0, 3).map(appt => (
                  <div key={appt.id} className="py-4 flex items-center justify-between gap-4 first:pt-0 last:pb-0">
                    ...
                    {appt.consultationId && (
                      <Link href={`/patient/consultations/${appt.consultationId}`} className="text-sm font-medium text-teal-700 hover:text-teal-800">
                        View
                      </Link>
                    )}
                  </div>
