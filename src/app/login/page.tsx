...
export default function LoginPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      try {
        ...
        if (profileError instanceof ApiError && profileError.code === "ONBOARDING_REQUIRED") {
  ...
  return (
    <AuthCard
      ...
      footer={{ text: "New to MediSync?", label: "Create an account", href: "/register" }}
    >
      {message ? <FormAlert message={message} /> : null}
      <form className="space-y-5" onSubmit={handleSubmit} noValidate>
        <label className="block text-sm font-medium text-slate-700">
          Email address
          <input className={inputClassName} type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" required />
        </label>
        <label className="block text-sm font-medium text-slate-700">
          Password
        ...
        <div className="text-right">
          <Link className="text-sm font-semibold text-teal-700 hover:text-teal-800" href="/forgot-password">Forgot password?</Link>
        </div>
