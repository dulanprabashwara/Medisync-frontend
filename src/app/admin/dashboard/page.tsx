...
type AdminTab =
  "doctors" | "pharmacists" | "hospitals" | "departments" | "specializations";
...
function AdminDashboardContent() {
  ...
  const [selectedDoctor, setSelectedDoctor] =
    useState<AdminDoctorReview | null>(null);
  const [selectedPharmacist, setSelectedPharmacist] =
    useState<AdminPharmacistReview | null>(null);
  ...
  async function saveHospital(event: FormEvent) {
    ...
    await runAction(
      "hospital-save",
      hospitalForm.id ? "Hospital updated." : "Hospital created.",
      async () => {
        const input = {
        ...
        if (hospitalForm.id) {
          await updateAdminHospital(session.access_token, {
            id: hospitalForm.id,
            ...input,
          });
        } else {
  ...
  async function saveDepartment(event: FormEvent) {
    event.preventDefault();
    if (!session || !departmentForm.hospitalId || !departmentForm.name.trim())
      return;
    await runAction(
      "department-save",
      ...
      async () => {
        ...
        if (departmentForm.id) {
          await updateAdminDepartment(session.access_token, {
    ...
    await runAction(
      ...
      async () => {
        ...
        if (specializationForm.id) {
          await updateAdminSpecialization(session.access_token, {
            id: specializationForm.id,
            ...input,
          });
        } else {
  ...
  async function approveDoctor(doctor: AdminDoctorReview) {
    if (
      !session ||
      !window.confirm(`Approve Dr. ${doctor.firstName} ${doctor.lastName}?`)
    )
      return;
    await runAction(
  ...
  async function rejectSelectedDoctor() {
    ...
    if (
      !window.confirm(
        `Reject Dr. ${selectedDoctor.firstName} ${selectedDoctor.lastName}'s submission?`,
      )
    )
      return;
    await runAction(
      `doctor-${selectedDoctor.doctorId}`,
      "Doctor submission rejected with feedback.",
      async () => {
        await rejectDoctor(
  ...
  async function approvePharmacist(pharmacist: AdminPharmacistReview) {
    if (
      !session ||
      !window.confirm(
        `Approve ${pharmacist.firstName} ${pharmacist.lastName} as a verified pharmacist?`,
      )
    )
      return;
    await runAction(
      `pharmacist-${pharmacist.pharmacistId}`,
      "Pharmacist verified and activated.",
      async () => {
  ...
  async function rejectSelectedPharmacist() {
    ...
    if (
      !window.confirm(
        `Reject ${selectedPharmacist.firstName} ${selectedPharmacist.lastName}'s submission?`,
      )
    )
      return;
    await runAction(
      `pharmacist-${selectedPharmacist.pharmacistId}`,
      "Pharmacist submission rejected with feedback.",
      async () => {
        await rejectPharmacist(
          session.access_token,
  ...
  if (loading)
    return <LoadingPanel label="Loading the administration portal..." />;

  return (
    <main className="mx-auto max-w-7xl px-6 py-10 lg:px-8 lg:py-14">
      <p className="text-xs font-bold uppercase tracking-[0.16em] text-teal-700">
        MediSync Administration
      </p>
      <h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">
        Welcome, {profile?.firstName ?? "Administrator"}
      </h1>
      <p className="mt-3 max-w-3xl leading-7 text-slate-600">
        Manage trusted reference data and review submitted doctor and pharmacist
      ...
      >
        <TabButton active={tab === "doctors"} onClick={() => setTab("doctors")}>
        ...
        >
          Pending pharmacists ({pharmacists.length})
        </TabButton>
        <TabButton
      ...
      {tab === "doctors" ? (
        <section className="mt-7">
          <SectionHeading
            ...
            description="Only complete, submitted profiles appear in this review queue."
          />
          {doctors.length === 0 ? (
            ...
            <div className="mt-5 grid gap-4 lg:grid-cols-2">
              {doctors.map((doctor) => (
                ...
                >
                  ...
                  <button
                    className="mt-5 rounded-xl border border-teal-700 px-4 py-2 text-sm font-semibold text-teal-700 hover:bg-teal-50"
                    onClick={() => {
      ...
      {tab === "pharmacists" ? (
        <section className="mt-7">
          <SectionHeading
            ...
            description="Review submitted professional registration and pharmacy information."
          />
          {pharmacists.length === 0 ? (
            <EmptyState message="No pharmacists are currently awaiting verification." />
          ) : (
            <div className="mt-5 grid gap-4 lg:grid-cols-2">
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-600">
                        {pharmacist.professionalRegistrationNumber}
                      </p>
                    </div>
                    <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-bold text-amber-800">
                      Pending
                  ...
                  <dl className="mt-5 grid gap-3 text-sm sm:grid-cols-2">
                    ...
                    <ReviewDetail
                      ...
                      value={
                        pharmacist.pharmacyRegistrationNumber || "Not provided"
                      }
                    />
                    <ReviewDetail
                      label="Submitted"
                      value={new Date(
                        pharmacist.submittedForVerificationAt,
                      ).toLocaleString()}
                    />
                  </dl>
                  <button
      ...
      {tab === "hospitals" ? (
        <MasterSection
          title="Hospitals"
          description="Create real hospital records and deactivate entries without deleting references."
          form={
            <HospitalForm
              value={hospitalForm}
              busy={busy === "hospital-save"}
              onChange={setHospitalForm}
              onSubmit={saveHospital}
              onCancel={() => setHospitalForm(emptyHospital)}
            />
          }
        >
          {hospitals.length === 0 ? (
            ...
            hospitals.map((hospital) => (
              <MasterRow
                ...
                title={hospital.name}
                subtitle={
                  [hospital.city, hospital.addressLine]
                    .filter(Boolean)
                    .join(" - ") || "No location provided"
                }
                active={hospital.active}
                busy={busy === `hospital-${hospital.id}`}
                doctorCount={hospital.doctorCount ?? 0}
                doctorsHref={`/admin/users?role=DOCTOR&hospitalId=${hospital.id}`}
                onEdit={() =>
                  setHospitalForm({
                    id: hospital.id,
                    name: hospital.name,
      ...
      {tab === "departments" ? (
        <MasterSection
          title="Departments"
          description="Every department belongs to exactly one hospital."
          form={
            <DepartmentForm
              value={departmentForm}
              hospitals={hospitals}
              busy={busy === "department-save"}
          ...
          {departments.length === 0 ? (
            ...
            departments.map((department) => (
              <MasterRow
                ...
                subtitle={department.hospitalName}
                active={department.active}
                busy={busy === `department-${department.id}`}
                doctorCount={department.doctorCount ?? 0}
                doctorsHref={`/admin/users?role=DOCTOR&departmentId=${department.id}`}
                onEdit={() =>
                  setDepartmentForm({
                    id: department.id,
                    hospitalId: department.hospitalId,
      ...
      {tab === "specializations" ? (
        <MasterSection
          ...
          description="These database-backed choices appear in doctor professional profiles."
          form={
            <SpecializationForm
              value={specializationForm}
              busy={busy === "specialization-save"}
              onChange={setSpecializationForm}
              onSubmit={saveSpecialization}
              onCancel={() => setSpecializationForm(emptySpecialization)}
            />
          }
        >
          {specializations.length === 0 ? (
            <EmptyState message="No specializations have been added yet." />
          ) : (
            specializations.map((specialization) => (
              <MasterRow
                key={specialization.id}
                title={specialization.name}
                subtitle={specialization.description || "No description"}
                active={specialization.active}
                busy={busy === `specialization-${specialization.id}`}
                doctorCount={specialization.doctorCount ?? 0}
                doctorsHref={`/admin/users?role=DOCTOR&specializationId=${specialization.id}`}
                onEdit={() =>
      ...
      {selectedDoctor ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Doctor verification review"
        >
          <section className="mx-auto max-w-3xl rounded-3xl bg-white p-6 shadow-2xl sm:p-8">
            ...
            <dl className="mt-7 grid gap-5 sm:grid-cols-2">
              ...
              <ReviewDetail
                ...
                value={selectedDoctor.medicalRegistrationNumber}
              />
              <ReviewDetail
                label="Years of experience"
                value={`${selectedDoctor.yearsOfExperience} years`}
              />
              <ReviewDetail
                label="Hospital"
              ...
              <ReviewDetail
                label="Specialization"
                value={selectedDoctor.specializationName}
              />
              <ReviewDetail
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              >
                {busy ? "Processing..." : "Approve doctor"}
              </button>
              <button
                className="rounded-xl bg-rose-700 px-5 py-3 text-sm font-semibold text-white disabled:opacity-60"
                disabled={busy !== null}
                onClick={() => void rejectSelectedDoctor()}
              >
                {busy ? "Processing..." : "Reject with reason"}
              </button>
            </div>
          </section>
        </div>
      ...
      {selectedPharmacist ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Pharmacist verification review"
        >
          <section className="mx-auto max-w-3xl rounded-3xl bg-white p-6 shadow-2xl sm:p-8">
            <div className="flex items-start justify-between gap-4">
              <div>
            ...
            <dl className="mt-7 grid gap-5 sm:grid-cols-2">
              ...
              <ReviewDetail
                label="Phone"
                value={selectedPharmacist.phone || "Not provided"}
              />
              <ReviewDetail
                label="Professional registration"
                value={selectedPharmacist.professionalRegistrationNumber}
              />
              <ReviewDetail
                label="Pharmacy"
                value={selectedPharmacist.pharmacyName}
              />
              <ReviewDetail
                label="Pharmacy registration"
                value={
                  selectedPharmacist.pharmacyRegistrationNumber ||
                  "Not provided"
                }
              />
              <ReviewDetail
                label="Qualifications"
                value={selectedPharmacist.qualifications || "Not provided"}
              />
            </dl>
            <div className="mt-5 rounded-2xl bg-slate-50 p-4">
              ...
              <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-700">
                {selectedPharmacist.pharmacyAddress}
              </p>
            </div>
            <label className="mt-6 block text-sm font-semibold text-slate-700">
              Rejection reason
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              >
                {busy ? "Processing..." : "Approve pharmacist"}
              </button>
              <button
                className="rounded-xl bg-rose-700 px-5 py-3 text-sm font-semibold text-white disabled:opacity-60"
                disabled={busy !== null}
...
function TabButton({
  ...
  return (
    <button
      className={`rounded-xl px-4 py-2.5 text-sm font-semibold ${active ? "bg-teal-700 text-white" : "border border-slate-300 bg-white text-slate-700"}`}
      onClick={onClick}
    >
      {children}
    </button>
...
function MasterSection({
  ...
  return (
    <section className="mt-7 grid gap-7 lg:grid-cols-[22rem_1fr]">
      <div>
...
function MasterRow({
  ...
  return (
    <article className="flex flex-col justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center">
      <div>
        <div className="flex items-center gap-2">
          <h3 className="font-semibold text-slate-950">{title}</h3>
          <span
            className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${active ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
          >
        ...
        >
          Doctors: {doctorCount} · View doctors
        </Link>
      </div>
...
function HospitalForm({
  ...
  value: HospitalFormValue;
  busy: boolean;
  onChange: (value: HospitalFormValue) => void;
  ...
  return (
    ...
    >
      <h3 className="font-semibold">
        {value.id ? "Edit hospital" : "Add hospital"}
      </h3>
      <input
...
function DepartmentForm({
  ...
  return (
    ...
    >
      <h3 className="font-semibold">
        {value.id ? "Edit department" : "Add department"}
      </h3>
      <select
        className={inputClassName}
        required
        ...
        onChange={(event) =>
          onChange({ ...value, hospitalId: event.target.value })
        }
      >
...
function SpecializationForm({
  ...
  value: SpecializationFormValue;
  busy: boolean;
  onChange: (value: SpecializationFormValue) => void;
  onSubmit: (event: FormEvent) => void;
  onCancel: () => void;
}) {
  return (
    ...
    >
      <h3 className="font-semibold">
        {value.id ? "Edit specialization" : "Add specialization"}
      </h3>
      <input
        className={inputClassName}
