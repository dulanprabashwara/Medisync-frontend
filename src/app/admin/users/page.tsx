      ...
      >
        ...
        <FilterSelect
          ...
          onChange={(value) =>
            setFilters((current) => ({
              ...
              verificationStatus:
                value === "DOCTOR" || value === "PHARMACIST"
                  ? current.verificationStatus
                  : undefined,
              hospitalId: value === "DOCTOR" ? current.hospitalId : undefined,
              departmentId:
                value === "DOCTOR" ? current.departmentId : undefined,
              specializationId:
                value === "DOCTOR" ? current.specializationId : undefined,
              page: 0,
            }))
          }
        />
        <FilterSelect
          ...
          onChange={(value) =>
            setFilters((current) => ({
              ...current,
              status: (value || undefined) as AccountStatus | undefined,
              page: 0,
            }))
          }
        />
        ...
        {filters.role === "DOCTOR" || filters.role === "PHARMACIST" ? (
          <FilterSelect
            value={filters.verificationStatus ?? ""}
            label="All professional verification"
            values={["PENDING", "VERIFIED", "REJECTED"]}
            onChange={(value) =>
              setFilters((current) => ({
                ...current,
                verificationStatus: (value || undefined) as
        ...
        {filters.role === "DOCTOR" ? (
          <>
            <NamedFilter
              ...
              label="All hospitals"
              values={hospitals.map((item) => [item.id, item.name])}
              onChange={(value) =>
                setFilters((current) => ({
                  ...current,
            ...
            <NamedFilter
              ...
              onChange={(value) =>
                setFilters((current) => ({
                  ...current,
                  specializationId: value || undefined,
                  page: 0,
                }))
              }
            />
            ...
            <table className="w-full min-w-212.5 text-left text-sm">
              <thead className="bg-slate-50 text-slate-600">
                <tr>
                  ...
                  <th className="p-4">Professional profile</th>
              ...
              <tbody>
                {users.map((user) => (
                  <tr key={user.id} className="border-t border-slate-100">
                    ...
                    <td className="p-4">{user.verificationStatus ?? "—"}</td>
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      {new Date(user.lastRecordedActivityAt).toLocaleString()}
                    </td>
                    <td className="p-4">
                      <Link
...
function Professional({ user }: { user: AdminUserSummary }) {
  if (user.role === "DOCTOR")
    return (
      <span>
        {[user.hospitalName, user.departmentName, user.specializationName]
          .filter(Boolean)
          .join(" · ") || "—"}
      </span>
    );
  if (user.role === "PHARMACIST")
    return <span>{user.pharmacyName || "—"}</span>;
  return <span>—</span>;
}

function Avatar({ user }: { user: AdminUserSummary }) {
  return (
    <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full bg-teal-100 font-bold text-teal-900">
...
function Status({ value }: { value: AccountStatus }) {
  return (
    <span
      className={`rounded-full px-3 py-1 text-xs font-bold ${value === "ACTIVE" ? "bg-emerald-100 text-emerald-800" : value === "BANNED" || value === "DELETED" ? "bg-rose-100 text-rose-800" : "bg-amber-100 text-amber-900"}`}
    >
      {value}
    </span>
  );
