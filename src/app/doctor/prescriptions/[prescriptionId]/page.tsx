...
function Content() {
  ...
  const apply = useCallback((next: DoctorPrescription) => { setValue(next); setForm({ validityDays: next.validityDays, generalInstructions: next.generalInstructions ?? "", items: next.items.map((item) => ({ medicineName: item.medicineName, strength: item.strength, medicineForm: item.medicineForm, dosage: item.dosage, frequency: item.frequency, duration: item.duration, quantity: item.quantity, route: item.route, instructions: item.instructions })) }); setDirty(false); }, []);
