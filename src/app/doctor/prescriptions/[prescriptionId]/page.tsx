...
function Content() {
  ...
  const apply = useCallback((next: DoctorPrescription) => {
    setValue(next);
    setForm({
      validityDays: next.validityDays,
      generalInstructions: next.generalInstructions ?? "",
      doctorFeeAmount: next.doctorFeeAmount,
  ...
  function validateDraft() {
    ...
    if (
      !Number.isInteger(form.validityDays) ||
      form.validityDays < 1 ||
      form.validityDays > 90
    ) {
    ...
    if (Object.keys(nextErrors).length > 0) {
      setError(
        "Complete the highlighted required fields before saving this draft.",
      );
      setNotice(null);
      return false;
    }
  ...
  async function save() {
    ...
    try {
      ...
      setError(requestMessage(e, "The draft could not be saved."));
    } finally {
      setBusy(null);
    }
  }
  ...
  async function cancel() {
    ...
    const reason = window.prompt(
      "Cancellation reason (required, at least 3 characters):",
    );
    if (!reason) return;
    setBusy("cancel");
    setError(null);
    try {
      apply(
        await cancelPrescription(session.access_token, prescriptionId, reason),
      );
      setNotice("Prescription cancelled and its QR revoked.");
    } catch (e) {
      setError(
        e instanceof Error
          ? e.message
  ...
  async function discard() {
    if (
      !session ||
      !window.confirm("Discard this draft prescription? This cannot be undone.")
    )
      return;
    setBusy("discard");
    setError(null);
    try {
      await discardPrescriptionDraft(session.access_token, prescriptionId);
      setDirty(false);
      router.push("/doctor/prescriptions");
    } catch (e) {
      setError(
        e instanceof Error ? e.message : "The draft could not be discarded.",
      );
      setBusy(null);
    }
  }
  if (!value && !error)
    return <LoadingPanel label="Loading prescription editor..." />;
  const draftWritable =
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-10 lg:px-8 lg:py-14">
      <PortalHeading
        ...
        description="Create structured medication instructions and issue them when the consultation permits."
      ...
      {value ? (
        <>
          ...
          {value.status === "ISSUED" && value.doctorFeeAmount > 0 ? (
            <section
              className={`mt-6 rounded-2xl border p-5 ${value.doctorFeeStatus === "CONFIRMED" ? "border-emerald-200 bg-emerald-50 text-emerald-950" : "border-amber-200 bg-amber-50 text-amber-950"}`}
            >
              <p className="text-xs font-bold uppercase tracking-[0.16em]">
                Consultation fee
              </p>
              <h2 className="mt-2 text-2xl font-semibold">
                {value.doctorFeeCurrency}{" "}
                {Number(value.doctorFeeAmount).toFixed(2)}
              </h2>
              <p className="mt-2 text-sm">
                {value.doctorFeeStatus === "CONFIRMED"
                  ? `Payment confirmed${value.doctorFeeConfirmedAt ? ` ${new Date(value.doctorFeeConfirmedAt).toLocaleString()}` : ""}.`
                  : "Awaiting confirmation. The patient's QR remains locked until payment is confirmed in the consultation room."}
              </p>
              <div className="mt-4 border-t pt-4 text-sm">
                <p className="font-semibold">
                  Doctor payment details for this prescription:
                </p>
                <p className="mt-1">
                  Account Holder: {value.doctorBankAccountHolder || "N/A"}
                </p>
                <p>Bank: {value.doctorBankName || "N/A"}</p>
                <p>Branch: {value.doctorBankBranch || "N/A"}</p>
                <p>Account Number: {value.doctorBankAccountNumber || "N/A"}</p>
              </div>
            </section>
          ) : null}
          {value.dispensingStatus === "DISPENSED" ? (
            <section className="mt-6 rounded-2xl border border-sky-200 bg-sky-50 p-5 text-sky-950">
              <h2 className="font-semibold">Dispensing status: Dispensed</h2>
              {value.dispensedAt ? (
                <p className="mt-2 text-sm">
                  Dispensed {new Date(value.dispensedAt).toLocaleString()}
                </p>
              ) : null}
          ...
          {value.status === "DRAFT" ? (
            draftWritable ? (
              <section className="mt-6 space-y-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                ...
                <label className="block text-sm font-medium">
                  ...
                  />
                  {validationErrors.doctorFeeAmount ? (
                    <span className="mt-1 block text-xs text-rose-700">
                      {validationErrors.doctorFeeAmount}
                    </span>
                  ) : (
                    <span className="mt-1 block text-xs text-slate-500">
                      Use 0 when no manual payment confirmation is required. The
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-xl font-semibold">
                      Medicines ({form.items.length}/20)
                    </h2>
                    {dirty ? (
                  ...
                  <button
                    type="button"
                    className="rounded-xl border border-teal-700 px-4 py-2 text-sm font-semibold text-teal-800 disabled:opacity-50"
                    disabled={form.items.length >= 20 || busy !== null}
                    onClick={() => {
                      setDirty(true);
                      setForm((current) => ({
                        ...current,
                ...
                {form.items.map((item, index) => (
                  <fieldset
                    className="grid gap-4 rounded-2xl border border-slate-200 p-5 sm:grid-cols-2"
                    key={index}
                  >
                    ...
                    {(
                      ...
                      const fieldError = validationErrors[`${index}.${field}`];
                      ...
                      return (
                        <label className="text-sm font-medium" key={field}>
                          ...
                          <input
                            ...
                            required={required}
                            className={`mt-1 block w-full rounded-xl border px-3 py-2 ${fieldError ? "border-rose-500 bg-rose-50" : "border-slate-300"}`}
                            value={item[field] ?? ""}
                            onChange={(e) =>
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <button
                    type="button"
                    className="rounded-xl border border-rose-300 px-5 py-3 text-sm font-semibold text-rose-700 disabled:opacity-50"
                    disabled={busy !== null}
              ...
              <section className="mt-6 rounded-3xl border border-amber-200 bg-amber-50 p-6 text-amber-950">
                <h2 className="font-semibold">Legacy draft is read-only</h2>
                <p className="mt-2 text-sm">
                  This consultation is{" "}
                  {value.consultationStatus.toLowerCase().replace("_", " ")}.
                  The draft cannot be edited or issued, but it can be safely
                  discarded.
                </p>
                ...
                <button
                  type="button"
                  className="mt-5 rounded-xl bg-rose-700 px-5 py-3 text-sm font-semibold text-white disabled:opacity-50"
                  disabled={busy !== null}
                  onClick={() => void discard()}
                >
                  {busy === "discard" ? "Discarding..." : "Discard draft"}
            ...
            <section className="mt-6 rounded-2xl border border-rose-200 bg-rose-50 p-5 text-rose-950">
              <h2 className="font-semibold">Prescription cancelled</h2>
              {value.cancelledAt ? (
                <p className="mt-2 text-sm">
                  Cancelled {new Date(value.cancelledAt).toLocaleString()}
                </p>
              ) : null}
              ...
              <p className="mt-3 text-sm">
                Medicine details and instructions are hidden for cancelled
                prescriptions.
              </p>
            </section>
          ) : (
            <>
              ...
              {value.generalInstructions ? (
                <p className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 whitespace-pre-wrap">
                  {value.generalInstructions}
                </p>
              ) : null}
              {value.cancellationAllowed ? (
                <button
                  type="button"
                  className="mt-6 rounded-xl bg-rose-700 px-5 py-3 text-sm font-semibold text-white disabled:opacity-50"
                  disabled={busy !== null}
                  onClick={() => void cancel()}
                >
                  {busy === "cancel" ? "Cancelling..." : "Cancel prescription"}
                </button>
              ) : value.cancellationBlockedReason ? (
