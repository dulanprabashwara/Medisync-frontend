...
import { ApiError, cancelPrescription, confirmPrescriptionPayment, discardPrescriptionDraft, getDoctorPrescription, issuePrescription, updatePrescriptionDraft } from "@/lib/api";
...
function Content() {
  ...
  async function confirmPayment() { if (!session || !window.confirm("Confirm that you received this consultation fee? This action is recorded in the audit log.")) return; setBusy("payment"); setError(null); setNotice(null); try { apply(await confirmPrescriptionPayment(session.access_token, prescriptionId)); setNotice("Consultation fee confirmed. The patient can now generate the prescription QR."); } catch (e) { setError(e instanceof Error ? e.message : "Payment could not be confirmed."); } finally { setBusy(null); } }
