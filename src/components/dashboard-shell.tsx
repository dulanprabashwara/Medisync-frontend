  ...
  return (
    <div className="py-2">
      ...
      <section
        ...
        aria-label={`${role.toLowerCase()} portal modules`}
      >
        {modules.map((module) => {
          const content = (
            <>
              <div className="flex items-start justify-between gap-4">
                <span className="grid size-10 place-items-center rounded-xl bg-teal-50 font-bold text-teal-700">
                  +
                </span>
          ...
          return module.href ? (
            <Link
              className="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm hover:border-teal-300 hover:shadow-md transition-colors"
              href={module.href}
              key={module.title}
            >
              {content}
